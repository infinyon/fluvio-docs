...
The `sdf deploy` execute a dataflow in the worker. 
...
* `--skip-dataflow` skips deploying current dataflow  in the directory.


Once deployment is done, the `deploy` command opens interactive shell to worker.
