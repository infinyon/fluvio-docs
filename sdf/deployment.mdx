...
To list all dataflow running in the worker, you can use `show dataflow` command which shows fully qualified name of the dataflow and status.
...
To select the dataflow, you can use `dataflow select` with fully qualified dataflow name.
...
In certain case, you want to stop the dataflow but not delete it.  You can use `stop` command.
