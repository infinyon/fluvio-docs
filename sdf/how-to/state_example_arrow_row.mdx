...
This tutorial is a continuation from the previous state example. This tutorial shows how to use arrow-rows to store more complicated data types not available in primitive key-values. Read more about [arrow rows][arrow]. We will continue from the tutorial from the [merge example][merge] to have a saved balance.
