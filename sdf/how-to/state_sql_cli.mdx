...
### Overview
From previous examples we learned how to create and manipulate a state. Here we will do the same but also will use CLI interface to access state data.
In particular, we will simulate a dataflow in which we get sensor data from a vehicle in a topic and then we can query the data of each sensor using SQL. 
