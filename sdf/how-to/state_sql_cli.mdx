...
```
#### 3. Updating State
To update the state in an `arrow-row`, we need to update the individual row's columns manual and call an `update()`. We can also use `delete()` to purge the state on the key.
```YAML
partition:
  assign-key:
    (...)
