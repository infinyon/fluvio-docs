...
Beta2 CLI is not backward compatible with Beta1 workers.  Please upgrade workers to Beta2 in order to use the Beta2 CLI.
...
deployment changes, see [deployment]:
`sdf run` -> `sdf deploy`,  
`sdf run --ephemeral` -> `sdf run`
