...
Powered by WebAssembly (also called wasm), [SmartModules](/smartmodules) are pre-packaged or user-provided operations such as filters, maps, or aggregators that can be applied to records at various points in the streaming pipeline.
...
Use Connectors locally with [`cdk`](/connectors/cdk) or start a connector on [InfinyOn Cloud](https://infinyon.cloud), and let us manage the infrastruture
