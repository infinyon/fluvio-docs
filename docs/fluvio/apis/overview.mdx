...
* `max_request_size`: Maximum amount of bytes that the producer can send in a single request. If the record is larger than the max request size, the producer will fail to send the record. Defaults to 1048576 bytes.
* `batch_size`: Maximum amount of bytes accumulated by the records before sending the batch. If the record is larger than the batch size, the producer will send the record in a single new batch. Defaults to 16384 bytes.
