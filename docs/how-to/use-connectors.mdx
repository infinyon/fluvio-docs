...
Ensure `fluvio` is installed and logged in to InfinyOn Cloud before proceeding. If not set up, follow the [Quick Start](/docs/quickstart).
...
Below is the configuration for the [Inbound HTTP connector][inbound-http-connector]
